...
独立工作流 **SL-3000 Security Monitor** 每天北京时间 09:17 检查，每周一发送汇总，也支持手动触发并强制发送报告。它不会修改锁文件、构建或刷写固件。新增安全修复关键词线索和新增/修订的高危公告会推送 Telegram；普通版本变化放入周报。首次扫描包含历史公告，可能包含已修复或未安装组件的问题，不能据此认定设备受影响。
...
- 默认基准为仓库当前锁文件，报告明确标注实机版本未核实。可设置仓库 Actions Variable `SL3000_DEPLOYED_RECIPE` 为实际已刷入固件的完整配方提交 SHA，工作流会读取该提交的锁文件；这仍是人工登记，机器人不连接路由器验证软件和配置。
...
- 默认分支估算 **45、55、59 天无提交**时分别提醒，周报持续列出无提交天数。GitHub 的公开仓库定时任务可能在 60 天无仓库活动后停用；提交时间只是保守参考，其他仓库活动和推送时间可能影响实际期限。发现新的默认分支提交后重新计时，不自动创建保活提交。
