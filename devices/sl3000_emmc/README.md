...
2026-09-08 安全更新通过锁文件的 `package_updates` 单独覆盖相关包配方，OpenWrt、内核及其余 feed 基线仍使用上述提交：

| 组件 | 纳入的版本 | 修复来源 |
| --- | --- | --- |
| uhttpd | `2026.08.03~60f64bec-r2` | 官方包更新 `80de19ea3b6e`，另回移 `3aa7e1c76782` 的 Basic Auth 修复；覆盖报告中的 8 条修复 |
| rpcd | `2026.07.19~e37ed9d8-r1` | 官方包更新 `029fec82456a`，修复符号链接绕过文件 ACL |
| cgi-io | `2026.07.21~31cb3c89-r1` | 官方包更新 `f36e3e98df44`，修复异常 POST 解码后的释放后使用 |
| Mbed TLS | `3.6.7-r1` | 官方包更新 `390a867118e1`，包含本次 LTS 安全修复并移除已合并的旧补丁 |
| nghttp2 | `1.70.0-r1` | 官方包更新 `fc8b2fec0ba9`，包含 CVE-2026-27135 修复 |
| Tailscale | `1.102.3-r1` | GitHub 官方 Latest，`prerelease=false`；沿用发行分支包配方，锁定稳定版本与源码包 SHA-256 |
| Go 构建工具链 | `1.26.6` | 官方发行分支更新 `af41897f31a4`，满足 Tailscale 的 `go.mod` 要求；编译器不安装到路由器 |

准备阶段只导入这些包目录，重新生成相关 feed 索引，并核对完整目录内容及回移补丁的 SHA-256。产物检查要求包清单中的版本与锁文件一致；公共安全清单再次验证实际源码。监控使用单独锁定的包来源和已验证回移记录继续追踪后续修复。更改锁定源码后，需完成匹配的新构建才能生成新的有效检查基准；构建完成不代表路由器已经刷入。

锁文件 `hardening` 声明 LuCI 挂载权限修复 `GHSA-v5f9-62c7-cw29`。`hardening.py` 用 JSON 解析删除 `luci-mod-system-mounts` 对 `/etc/crontabs/root` 的多余写权限，保留正常挂载操作和独立的 cron 管理权限。产物检查从实际 SquashFS 读取 ACL 再次验证，公共安全清单记录已验证的修复和 Go 工具链版本；仅修改报告状态不能让检查通过。
...
固件内置锁定 feed 中的 vnStat 2.13、vnstati2、LuCI 页面和中文翻译。默认分别统计有线 `wan`、当前无线 WAN `phy1-sta0` 和 `tailscale0`；未连接的接口没有流量。更换上网接口后，在 LuCI 的 vnStat 设置中调整统计接口。vnStat 读取接口累计计数，不提供按客户端识别或限速；Tailscale 流量也经过上网接口，两者不能相加作为总上网流量。

数据库位于 eMMC overlay 的 `/etc/vnstat/`，每 15 分钟保存一次，正常停止服务时也会保存。重启后保留历史；意外断电可能损失尚未保存的采样。目录权限为 `0700`，不额外监听网络端口。配置通过 rootfs overlay 注入，因此本次从未安装 vnStat 的固件保留设置升级，也能获得这些默认值；后续升级尊重已保存的 vnStat 配置。

`/lib/upgrade/keep.d/sl3000-vnstat` 将默认数据库目录纳入保留配置升级和配置备份。升级或导出备份前先执行 `/etc/init.d/vnstat stop`，让计数落盘并避免在 SQLite 写入中途打包；若只导出备份，完成后执行 `/etc/init.d/vnstat start`。备份仍需保存在路由器以外；不保留配置或恢复出厂不会保留历史。自行更改数据库目录时，也应相应修改备份路径。

CI 同时核对最终包清单、镜像内配置和数据库备份规则。首次安装后仍需确认 LuCI 能显示实际流量，以及重启前后的历史连续性。
...
- 检查 `sources.lock.json` 中的官方同系列分支、第三方分支及正式版本，同时读取脚本所列上游仓库公开的 GitHub 安全公告，以及 Tailscale 官方安全公告 RSS。RSS 使用 XML/HTML 解析，保留明确修复版本和适用条件；读取或解析失败会报告检查不完整。其他厂商公告、邮件列表、传递依赖等未全部覆盖；版本差异和关键词匹配只是待核实线索，报告不会声称“固件无漏洞”。
...
- `security-policy.json` 保存明确的使用约定，当前为“只有 root 管理员，没有委派 LuCI 管理账号”，只排除已复核且依赖该条件的公告。Tailscale SSH、Serve/Funnel、Services、4via6 等条件未声明时保持未知，不根据默认关闭或没有目标端 Go 编译器推断不受影响。以后启用委派管理或这些功能时应更新此文件；`true` 表示启用，`false` 表示明确未启用，省略或 `null` 表示未知。报告列出假设及未解决的条件，手动结果也显示待核实公告数量；配置不适用与构建确实已修复分开标记。
